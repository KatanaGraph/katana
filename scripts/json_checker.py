...
def compare(resultfile, truthfile):
    ...
    try:
        ...
        if not correct:
            ...
            logging.error(
                "only in results: %s of %s items (first 10):\n%s",
                len(left),
                len(result_dicts),
            ...
            logging.error(
                "only in truth: %s of %s items (first 10):\n%s",
                len(right),
                len(truth_dicts),
...
if __name__ == "__main__":
    ...
    parser.add_argument("-querylist", help="list of queries being tested")
    parser.add_argument("-resultprefix", help="prefix for files containing json of results")
    parser.add_argument("-truthdir", help="directory containing json files of ground truth")

    parsed_arguments = parser.parse_args()

    print("Starting comparison...")
    ret = main(
        parsed_arguments.querylist, parsed_arguments.resultprefix, parsed_arguments.truthdir, parsed_arguments.delete
    )
