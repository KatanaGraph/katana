...
#ifndef KATANA_LIBGRAPH_KATANA_MORPHSEPINOUTGRAPH_H_
#define KATANA_LIBGRAPH_KATANA_MORPHSEPINOUTGRAPH_H_
 ...
 * g.getEdgeData(jj); KATANA_LOG_DEBUG_ASSERT(edgeData == 5);
 *   }
 * }
 * \endcode
 *
  ...
  class gNode : public internal::NodeInfoBase<NodeTy, !HasNoLockable>,
    ...
    iterator find(gNode* N, bool inEdge = false) {
      ...
      if (SortedNeighbors) {
        ...
        ii = std::lower_bound(
            edgelist.begin(), edgelist.end(), N, first_lt<gNode*>());
      } else {
        ii = edgelist.begin();
      }
...
public:
  ...
  in_edge_iterator in_edge_begin(
    ...
    return boost::make_filter_iterator(
        is_in_edge(), N->in_edge_begin(), N->in_edge_end());
  }
  ...
  in_edge_iterator in_edge_end(
    ...
    // N->acquire(mflag);
    return boost::make_filter_iterator(
        is_in_edge(), N->in_edge_end(), N->in_edge_end());
  }
