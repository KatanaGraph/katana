...
from galois.atomic import GAccumulator, GReduceLogicalOr, atomic_add, GReduceMin, GReduceMax
...
def pagerank_pull_sync_residual(graph: PropertyGraph, maxIterations, tolerance, property_name):
    ...
    do_all(range(num_nodes),
               ...
               steal=True, loop_name="initialize_pagerank_pull_residual")

    # Compute out-degree for each node
    do_all(range(num_nodes),
               compute_out_deg_operator(graph, nout.as_numpy()),
               steal=True, loop_name="Compute_out_degree")
    ...
    while iterations < maxIterations and changed.reduce():
        ...
        iterations += 1 
        do_all(range(num_nodes),
               compute_pagerank_pull_delta_operator(graph, rank.as_numpy(), nout.as_numpy(), delta.as_numpy(), residual.as_numpy(), tolerance, changed),
               steal=True, loop_name="pagerank_delta")

        do_all(range(num_nodes),
               compute_pagerank_pull_residual_operator(graph, delta.as_numpy(), residual.as_numpy()),
               steal=True, loop_name="pagerank")
...
@do_all_operator()
def sanity_check_operator(sum_rank: GAccumulator[int], max_rank: GReduceMax[int], min_rank: GReduceMin[int], data, nid):
    val = data[nid]
    sum_rank.update(val)
    max_rank.update(val)
    min_rank.update(val)
