...
class CommandError(subprocess.CalledProcessError):
    def __init__(self, *args):
        if len(args) == 1 and isinstance(args[0], subprocess.CalledProcessError):
            super().__init__(args[0].returncode, args[0].cmd, args[0].output, args[0].stderr)
        else:
            super().__init__(*args)
...
def predicate_command(*args, ignore_error=False, **kwargs) -> bool:
    try:
        res = subprocess.run(args, check=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, **kwargs)
    except subprocess.CalledProcessError as e:
        raise CommandError(e) from e
    out = res.stdout.decode("utf-8").strip("\n")
