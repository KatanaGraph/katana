...
def bfs(graph: Graph, input_args, source_node_file=""):
    ...
    if source_node_file:
        ...
        for source in sources:
            ...
            time_data[f"bfs-schema-check-on-{source}"] = time.time() - \
                start_schema_check
...
def sssp(graph: Graph, input_args, source_node_file=""):
    ...
    if source_node_file:
        ...
        for source in sources:
            ...
            start_schema_check = time.time()
            check_schema(graph, property_name)
            time_data[f"sssp-schema-check-on-{source}"] = time.time() - \
                start_schema_check
        ...
        start_schema_check = time.time()
        check_schema(graph, property_name)
        time_data[f"sssp-schema-check"] = time.time() - \
            start_schema_check
...
def jaccard(graph: Graph, input_args):
    ...
    start_schema_check = time.time()
    check_schema(graph, property_name)
    time_data["jaccard-schema-check"] = time.time() - \
        start_schema_check
...
def pagerank(graph: Graph, _input_args):
    ...
    start_schema_check = time.time()
    check_schema(graph, property_name)
    time_data["pagerank-schema-check"] = time.time() - \
        start_schema_check
...
def bc(graph: Graph, input_args, source_node_file="", num_sources=4):
    ...
    if source_node_file:
        ...
        for run in range(0, runs):
            ...
            start_schema_check = time.time()
            check_schema(graph, property_name)
            time_data["bc-schema-check"] = time.time() - \
                start_schema_check
...
def cc(graph: Graph, _input_args):
    ...
    start_schema_check = time.time()
    check_schema(graph, property_name)
    time_data["cc-schema-check"] = time.time() - \
        start_schema_check
...
def kcore(graph: Graph, _input_args):
    ...
    start_schema_check = time.time()
    check_schema(graph, property_name)
    time_data["k-core-schema-check"] = time.time() - \
        start_schema_check
...
def louvain(graph: Graph, input_args):
    ...
    start_schema_check = time.time()
    check_schema(graph, property_name)
    time_data["louvain-schema-check"] = time.time() - \
        start_schema_check
...
def run_routine(routine, data, load_time, args_trails, argv):
    ...
    for _ in range(args_trails):
        ...
        data["routines"][f"{str(routine.__name__)}_{str(trial_count)}"] = convert_to_milisec(
            time_data)
...
def run_all_gap(args):
    ...
    if args.application not in ["all"]:
        ...
        for k in routine_name_args_mappings:
            ...
            data = run_routine(
                routine_name_args_mappings[k].name, data, load_time, args.trials, routine_args)

    if args.save_json:
        save_success = save_statistics_as_json(
            data, start_time, args.file_name, args.save_dir)
