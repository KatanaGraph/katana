...
curl https://apache.jfrog.io/artifactory/arrow/$(lsb_release --id --short | tr 'A-Z' 'a-z')/apache-arrow-apt-source-latest-$(lsb_release --codename --short).deb --output /tmp/apache-arrow-apt-source-latest.deb
...
run_as_original_user pip3 install --no-binary pyarrow 'pyarrow>=4.0<5'
