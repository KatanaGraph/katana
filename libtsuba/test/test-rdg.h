...
katana::Result<katana::URI>
find_file(const katana::URI& search_path, const std::string& substring) {
  KATANA_LOG_VASSERT(
      search_path.scheme() == "file",
      "Function cannot handle paths that are not local files");

  KATANA_LOG_DEBUG("finding file matching {}", substring);
  const std::filesystem::directory_iterator end;
  try {
    for (std::filesystem::directory_iterator iter{search_path.path()};
         iter != end; iter++) {
      const std::string file_name = iter->path().filename().string();
      if (std::filesystem::is_regular_file(*iter)) {
        if (file_name.find(substring) != std::string::npos) {
          return katana::URI::Make(iter->path().string());
        }
      }
    }
  } catch (std::exception&) {
