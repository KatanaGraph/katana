...
def run_all_gap(args):
    ...
    if args.application in ["bfs", "sssp", "bc", "jaccard"]:
        ...
        if args.application == "bfs":
            data = run_rootine(run_bfs, data, args.trials, (graph, input, args.source_nodes))

        if args.application == "sssp":
            data = run_rootine(run_sssp, data, args.trials, (graph, input, args.source_nodes))

        if args.application == "jaccard":
            data = run_rootine(run_jaccard, data, args.trials, (graph, input))

        if args.application == "bc":
            data = run_rootine(run_bc, data, args.trials, (graph, input, args.source_nodes, 4))
