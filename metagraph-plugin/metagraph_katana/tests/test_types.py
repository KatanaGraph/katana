...
def test_topology(kg_rmat15_cleaned_di):
    ...
    assert [kg_rmat15_cleaned_di.value.get_edge_dest(i) for i in kg_rmat15_cleaned_di.value.edges(0)][0:5] == [
        1,
        2,
        3,
        4,
    ...
    assert [kg_rmat15_cleaned_di.value.get_edge_dest(i) for i in kg_rmat15_cleaned_di.value.edges(8)][0:5] == [
        0,
        9,
        10,
        11,
...
def test_topology(nx_weighted_undirected_8_12, nx_weighted_directed_8_12):
    assert list(nx_weighted_undirected_8_12.value.nodes(data=True)) == list(
        nx_weighted_directed_8_12.value.nodes(data=True)
    )
    assert list(nx_weighted_undirected_8_12.value.nodes(data=True)) == [
        (0, {}),
        (1, {}),
        (3, {}),
        (4, {}),
    ...
    assert list(nx_weighted_undirected_8_12.value.edges(data=True)) == [
        (0, 1, {"weight": 4}),
        (0, 3, {"weight": 2}),
        (0, 4, {"weight": 7}),
        (1, 3, {"weight": 3}),
    ...
    assert list(nx_weighted_directed_8_12.value.edges(data=True)) == [
        (0, 1, {"weight": 4}),
        (0, 3, {"weight": 2}),
        (0, 4, {"weight": 7}),
        (1, 3, {"weight": 3}),
