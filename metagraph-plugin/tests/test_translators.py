...
def test_num_nodes(nx_from_kg_di_8_12, katanagraph_cleaned_8_12_di):
    ...
    for nid in katanagraph_cleaned_8_12_di.value:
        if nid not in nlist:
            assert katanagraph_cleaned_8_12_di.value.edges(nid) == range(0, 0)
            num_no_edge_nodes += 1
    assert num_no_edge_nodes + len(nlist) == katanagraph_cleaned_8_12_di.value.num_nodes()
    assert num_no_edge_nodes == 0


def test_num_edges(nx_from_kg_di_8_12, katanagraph_cleaned_8_12_di):
    edge_dict_count = {
    ...
    for src in katanagraph_cleaned_8_12_di.value:
        for dest in [katanagraph_cleaned_8_12_di.value.get_edge_dest(e) for e in katanagraph_cleaned_8_12_di.value.edges(src)]:
