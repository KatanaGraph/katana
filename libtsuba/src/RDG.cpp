...
UnloadProperty(
  ...
  if (prop_info.IsDirty()) {
    ...
    prop_info.NoteWrite(path);
  }

  prop_info.NoteUnload();
...
katana::Result<std::shared_ptr<arrow::Table>>
LoadProperty(
    const std::shared_ptr<arrow::Table>& props, const std::string name,
    uint32_t i, std::vector<tsuba::PropStorageInfo>* prop_info_list,
    const katana::Uri& dir) {
  if (i > static_cast<uint32_t>(props->num_columns())) {
    i = static_cast<uint32_t>(props->num_columns());
  }
...
katana::Result<void>
tsuba::RDG::LoadNodeProperty(const std::string& name, uint32_t i) {
...
katana::Result<void>
tsuba::RDG::LoadEdgeProperty(const std::string& name, uint32_t i) {
  std::shared_ptr<arrow::Table> new_props = KATANA_CHECKED(LoadProperty(
