...
class PartialGraphView {
  std::shared_ptr<OutIndexView> outidx_;
  ...
  galois::Result<void> BindEdgeFile() {
    ...
    if (auto res =
            edge_file_.Bind(outidx_->filename(), edges_start, edges_stop);
        !res) {
      return res.error();
    }
...
public:
  ...
  PartialGraphView(std::shared_ptr<OutIndexView>&& outidx)
      : outidx_(std::move(outidx)) {}

  PartialGraphView(const std::shared_ptr<OutIndexView>& outidx)
      : outidx_(outidx) {}
  ...
  const std::shared_ptr<OutIndexView> out_index() const { return outidx_; }
