...
if has_katana:
    ...
    class KatanaGraph(GraphWrapper, abstract=Graph):
        def __init__(self, pg_graph, is_weighted=True, edge_weight_prop_name='value', is_directed=True, node_weight_index=None, node_dtype=None, edge_dtype='int'):
            super().__init__()
        ...
        class TypeMixin:
            ...
            def _compute_abstract_properties(cls, obj, props: Set[str], known_props: Dict[str, Any]) -> Dict[str, Any]:
                ...
                # fast props
                for prop in {'is_directed', 'node_type', 'node_dtype', 'edge_type', 'edge_dtype', 'edge_has_negative_weights'} - ret.keys():
                    if prop == 'is_directed':
                    ...
                    if prop == "node_dtype":
                        ret[prop] = (
                            None if obj.node_weight_index is None
                            else obj.node_dtype
                        )
                    if prop == 'edge_type':
                    ...
                    if prop == 'edge_has_negative_weights':
                        ret[prop] = False
                # slow props, only compute if asked
                slow_props = props - ret.keys()
                for prop in slow_props:
                    if prop == 'to do add some pg properties here':
                        ret[prop] = None
                    if prop == 'to do add some pg properties here 2':
                        ret[prop] = None
                return ret
            @classmethod
