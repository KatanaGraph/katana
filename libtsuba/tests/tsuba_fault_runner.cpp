...
int
CrashAndVerify(
    const std::string& uri_in, char* const* envp, const std::string& rl,
    const std::string& node_prop_num) {
  int status;
  pid_t len_pid;
  pid_t verify_pid;
  ...
  const char* const fault_len_argv[] = {
      ...
      nullptr};
  status = posix_spawn(
      &len_pid, fault_len_argv[0], NULL, NULL,
      const_cast<char* const*>(fault_len_argv), envp);
  if (status == 0) {
    if (waitpid(len_pid, &status, 0) != -1) {
      const char* const verify_argv[] = {
          "bin/tsuba_fault", "-v", uri_in.c_str(), NULL};
      status = posix_spawn(
          &verify_pid, verify_argv[0], NULL, NULL,
          const_cast<char* const*>(verify_argv), envp);
      if (status == 0) {
        if (waitpid(verify_pid, &status, 0) == -1) {
          GALOIS_LOG_WARN(
              "Verify waitpid: {}", galois::ResultErrno().message());
        }
      } else {
...
int
RunLenFaulty(
    const std::string& uri_in, char* const* envp, int64_t run_len_limit) {
