...
//
// public:
//  using NeighborsTy = galois::gstl::Vector<std::pair<unsigned,
//  QueryEdgeData>>;
//
...
//    std::sort(matchingOrderToVertexMap.begin(),
//    matchingOrderToVertexMap.end(),
//              orderQueryVertices);
//    for (VertexId i = 0; i < query_graph->size(); ++i) {
...
//  }
//
//  bool toAdd(unsigned n, const BaseEmbedding& emb, const VertexId dst,
//             unsigned index, const NeighborsTy& neighbors,
...
//                   ", dst = ", dst, "\n");
//    // galois::gDebug(", deg(d) = ", graph->degree(dst), ", deg(q) = ",
//    // query_graph->degree(pos+1));
//
//    if (pruneNode(query_graph, next_qnode, graph->getData(dst)))
//      return false;
//
//    // if the degree is smaller than that of its corresponding query vertex
//    if (!matchNodeDegree(*query_graph, next_qnode, *graph, dst))
//      return false;
//
...
//  void addEmbedding(unsigned n, const BaseEmbedding& emb, const VertexId dst,
//                    NeighborsTy& neighbors, unsigned& numInNeighbors,
//                    BaseEmbeddingQueue& out_queue) {
//    if (n <
//        max_size - 1) { // generate a new embedding and add it to the next
//        queue
...
//      }
//      total_num +=
//          1; // if size = max_size, no need to add to the queue, just
//          accumulate
//    }
//  }
//
...
//  }
//
//  unsigned pickNeighbor(const BaseEmbedding& emb, const NeighborsTy&
//  neighbors,
...
//
//  template <bool DFS, bool printEmbeddings = false>
//  void process_embedding(const BaseEmbedding& emb, NeighborsTy& neighbors,
//                         unsigned numInNeighbors,
//                         BaseEmbeddingQueue& out_queue) {
...
//
//  template <bool DFS, bool printEmbeddings = false>
//  inline void extend_vertex(BaseEmbeddingQueue& in_queue,
//                            BaseEmbeddingQueue& out_queue) {
//
//    unsigned numInNeighbors;
...
//        if (printEmbeddings)
//          queue.printout_embeddings(level, debug);
//        extend_vertex<false, printEmbeddings>(queue, queue2);
//        if (level == query_graph->size() - 1)
...
size_t subgraphQuery(QueryGraph&, QueryGraph&) {
  galois::StatTimer initTime("MiningInitTime");
  galois::gError("Pangolin integration is gone; need to fix");
  return 0;
