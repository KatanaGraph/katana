...
public:
  ...
  void addEmbedding(unsigned n, const BaseEmbedding& emb, const VertexId dst,
    ...
    if (n <
        max_size - 1) { // generate a new embedding and add it to the next queue
      ...
      total_num +=
          1; // if size = max_size, no need to add to the queue, just accumulate
    }
  }
  ...
  unsigned pickNeighbor(const BaseEmbedding& emb, const NeighborsTy& neighbors,
