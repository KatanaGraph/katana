...
katana::Result<katana::PropertyGraph::EntityTypeIDVec>
MapEntityTypeIDsArray(const tsuba::FileView& file_view) {
  ...
  katana::PropertyGraph::EntityTypeIDVec entity_type_id_array;
...
katana::PropertyGraph::EntityTypeIDVec
MakeDefaultEntityTypeIDVec(size_t vec_sz) {
  katana::PropertyGraph::EntityTypeIDVec type_ids;
...
GetEntityTypeIDsFromProperties(
  ...
  if (properties->num_rows() == 0) {
    return MakeDefaultEntityTypeIDVec(topo_size);
  }
...
katana::PropertyGraph::Make(
  ...
  if (rdg.EntityTypeIDsOutsideProperties()) {
    KATANA_LOG_DEBUG("loading EntityType data from outside properties");

    EntityTypeIDVec node_type_ids = KATANA_CHECKED(
...
katana::PropertyGraph::Make(katana::GraphTopology&& topo_to_assign) {
  return std::make_unique<katana::PropertyGraph>(
      ...
      MakeDefaultEntityTypeIDVec(topo_to_assign.num_nodes()),
      MakeDefaultEntityTypeIDVec(topo_to_assign.num_edges()),
...
katana::PropertyGraph::ConstructEntityTypeIDs() {
  ...
  if (rdg_.EntityTypeIDsOutsideProperties()) {
    KATANA_WARN_ONCE(
        "Type Info loaded from storage, so not constructing from properties");
    return katana::ResultSuccess();
  }
...
katana::PropertyGraph::DoWrite(
  ...
  return rdg_.Store(
      ...
      std::move(edge_entity_type_id_array_res), node_entity_type_manager_,
