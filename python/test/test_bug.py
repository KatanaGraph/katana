...
def test_bug_capture_environment():
    ...
    with NamedTemporaryFile(suffix=".zip") as fi:
        ...
        with zipfile.ZipFile(fi) as zipin:
            ...
            for fn in ["info/os.txt", "info/python.txt", "info/conda.txt", "info/cmake.txt", "etc/ld.so.conf"]:
                assert fn in files
            conda_txt: bytes = zipin.read("info/conda.txt")
            # Skip this test if we are testing a conda installed package.
            if b"katana-python" not in conda_txt and b"katana-enterprise-python" not in conda_txt:
